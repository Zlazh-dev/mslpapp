...
export function JadwalPrintPanel({ isOpen, onClose, hari }: JadwalPrintPanelProps) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Gagal mencetak PDF.');
        } finally {
            setGenerating(false);
        }
    };
    ...
    return (
        <>
            <div className={`fixed inset-0 bg-black/20 backdrop-blur-[2px] z-[250] transition-opacity duration-200 ${visible ? 'opacity-100' : 'opacity-0'}`} onClick={handleClose} />
            <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-white shadow-2xl z-[251] flex flex-col transition-transform duration-200 ease-out ${visible ? 'translate-x-0' : 'translate-x-full'}`}>
                {/* Header */}
